...
function Button({ children, secondary = false, onClick }: { children: React.ReactNode; secondary?: boolean; onClick?: () => void }) {
  return <button onClick={onClick} style={{ width: '100%', minHeight: 44, border: secondary ? `1px solid ${BORDER}` : 0, background: secondary ? RAISED : RED, color: TEXT, borderRadius: 14, padding: '12px 14px', fontWeight: 900, cursor: 'pointer', fontFamily: 'inherit' }}>{children}</button>;
}
...
export default function MobileReviewPage() {
  ...
  return (
    <div className="mobile-review-root" dir="rtl" style={{ position: 'fixed', inset: 0, zIndex: 9999, background: 'radial-gradient(circle at 25% 10%, #260B10 0, #0B0C0F 30%, #050607 72%)', overflow: 'auto', fontFamily: 'Tajawal, Arial, sans-serif', color: TEXT }}>
      <div className="mobile-review-layout" style={{ minHeight: '100%', display: 'grid', gridTemplateColumns: 'minmax(250px, 360px) minmax(340px, 430px)', gap: 34, alignItems: 'center', justifyContent: 'center', padding: 28 }}>
        <aside className="mobile-review-summary" style={{ maxWidth: 360 }}>
          ...
          <button onClick={() => setShowInfo((v) => !v)} style={{ minHeight: 44, marginTop: 20, background: 'transparent', border: `1px solid ${BORDER}`, color: TEXT, borderRadius: 12, padding: '10px 14px', cursor: 'pointer', fontFamily: 'inherit', fontWeight: 800 }}>{showInfo ? 'إخفاء ملاحظات النسخة' : 'عرض ملاحظات النسخة'}</button>
        ...
        <div className="mobile-review-device-column" style={{ justifySelf: 'center', width: '100%', maxWidth: 410 }}>
          <div style={{ background: '#030405', border: '1px solid #2C3037', borderRadius: 42, padding: 10, boxShadow: '0 34px 90px #00000088, 0 0 0 1px #ffffff08 inset' }}>
            <div style={{ height: 'min(790px, calc(100vh - 76px))', minHeight: 620, background: BG, borderRadius: 34, overflow: 'hidden', position: 'relative', display: 'flex', flexDirection: 'column' }}>
              ...
              <nav aria-label="التنقل الرئيسي لنسخة المراجعة" style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: '#0B0D11EE', backdropFilter: 'blur(18px)', borderTop: `1px solid ${BORDER}`, padding: '7px 8px 9px', display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 2 }}>{nav.map((item) => { const active = item.id === screen; return <button key={item.id} aria-label={item.label} aria-current={active ? 'page' : undefined} onClick={() => setScreen(item.id)} style={{ minHeight: 48, border: 0, background: 'transparent', color: active ? RED : MUTED, cursor: 'pointer', fontFamily: 'inherit', padding: '5px 3px', touchAction: 'manipulation' }}><div aria-hidden="true" style={{ fontSize: 18, lineHeight: 1 }}>{item.icon}</div><div style={{ fontSize: 8, marginTop: 5, fontWeight: active ? 950 : 700 }}>{item.label}</div></button>; })}</nav>
            </div>
          </div>
          <div style={{ display: 'flex', justifyContent: 'center', gap: 7, marginTop: 14 }}><button onClick={() => setScreen('campaign')} style={{ minHeight: 44, border: `1px solid ${BORDER}`, background: SURFACE, color: TEXT, borderRadius: 999, padding: '8px 12px', fontFamily: 'inherit', cursor: 'pointer', fontSize: 10 }}>الحملة</button><button onClick={() => setScreen('planner')} style={{ minHeight: 44, border: `1px solid ${BORDER}`, background: SURFACE, color: TEXT, borderRadius: 999, padding: '8px 12px', fontFamily: 'inherit', cursor: 'pointer', fontSize: 10 }}>المخطط</button><button onClick={() => setScreen('social')} style={{ minHeight: 44, border: `1px solid ${BORDER}`, background: SURFACE, color: TEXT, borderRadius: 999, padding: '8px 12px', fontFamily: 'inherit', cursor: 'pointer', fontSize: 10 }}>السوشيال</button></div>
        </div>
      </div>
      <style jsx>{`@media (max-width:850px){.mobile-review-layout{grid-template-columns:1fr!important;padding:0!important}.mobile-review-summary{display:none!important}.mobile-review-device-column{max-width:none!important;width:100%!important}}`}</style>
    </div>
  );
}
