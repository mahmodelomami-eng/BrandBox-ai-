...
function signedPayload(overrides: Record<string, unknown> = {}) {
  ...
  const payload = {
    orderReference: 'order-1', providerTxId: 'provider-1', status: 'paid', userId: '00000000-0000-4000-8000-000000000001',
    amountLYD: 25, currency: 'LYD', itemType: 'purchase', packageId: 'pkg_100', ...overrides
  };
  const raw = JSON.stringify(payload);
  return { raw, secret, signature: crypto.createHmac('sha256', secret).update(raw).digest('hex') };
}
...
async function run() {
  ...
  signed = signedPayload({ orderReference: 'order-sub', providerTxId: 'provider-sub', amountLYD: 145, itemType: 'subscription', packageId: undefined, planId: 'pro' });
