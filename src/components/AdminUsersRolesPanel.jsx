    ...
    <div dir="rtl" className="mx-auto max-w-[1500px] space-y-6 p-4 sm:p-6 lg:p-8 text-white">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-gray-400">إدارة الحسابات والأرصدة والحالات، مع شرح وظيفة كل دور وصلاحياته وقيوده قبل منحه لأي مستخدم.</p>
        </div>
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            {roles.filter((item) => item.role !== 'USER').map((item) => (
              <article key={item.role} className="rounded-2xl border border-white/10 bg-[#0f1118] p-5">
                ...
                <div className="mt-4 border-t border-white/5 pt-3 text-[11px] text-gray-500">{item.legacy ? 'دور قديم للتوافق فقط، لا يوصى بمنحه لحسابات جديدة.' : item.assignableByActor ? 'يمكنك تعيين هذا الدور وفق سياسة الصلاحيات الحالية.' : item.assignable ? 'الدور قابل للتعيين لكن حسابك الحالي لا يملك صلاحية منحه.' : 'غير قابل للتعيين.'}</div>
              </article>
            ))}
          </div>
        </div>
      ...
      {roleModal && selectedUser && <div className="fixed inset-0 z-[200] grid place-items-center bg-black/75 p-4"><div className="w-full max-w-3xl rounded-3xl border border-white/10 bg-[#0d0f15] p-6 shadow-2xl"><div className="flex items-center gap-2 text-lg font-black"><ShieldQuestion className="text-amber-300" /> تعيين أو إزالة دور إداري</div><p className="mt-2 text-sm text-gray-400">المستخدم: {selectedUser.email}</p><div className="mt-5 grid max-h-[58vh] gap-3 overflow-y-auto">{roleChoices.map((item) => <button key={item.role} onClick={() => setSelectedRole(item.role)} className={`rounded-2xl border p-4 text-right ${selectedRole === item.role ? 'border-[#FF2E4C] bg-[#FF2E4C]/10' : 'border-white/10 bg-white/[.02]'}`}><div className="font-black">{item.role === 'USER' ? 'إزالة الصلاحيات الإدارية — ' : ''}{item.labelAr} <span className="text-xs font-normal text-gray-500">({item.labelEn})</span></div><p className="mt-2 text-xs leading-5 text-gray-400">{item.descriptionAr}</p><div className="mt-3 grid gap-2 sm:grid-cols-2"><div className="rounded-xl bg-emerald-500/5 p-2 text-[11px] text-gray-400"><span className="font-bold text-emerald-300">يسمح له:</span> {(item.responsibilitiesAr || []).slice(0, 2).join(' · ')}</div><div className="rounded-xl bg-red-500/5 p-2 text-[11px] text-gray-400"><span className="font-bold text-red-300">لا يسمح له:</span> {(item.restrictionsAr || []).slice(0, 2).join(' · ')}</div></div><div className="mt-2 text-[10px] text-gray-500">الصلاحيات: {(item.permissions || []).slice(0, 8).join(' · ') || 'لا توجد صلاحيات إدارية'}</div></button>)}</div><div className="mt-5 rounded-xl border border-amber-500/20 bg-amber-500/5 p-3 text-xs leading-5 text-amber-100/80">تغيير الدور عملية إدارية حساسة وتُسجل في Audit Log. لا يمكن تعيين OWNER من هذه الواجهة، كما يمنع النظام خفض آخر SUPER_ADMIN أو تجاوز صلاحيات الحساب المنفذ.</div><div className="mt-5 flex justify-end gap-2"><button onClick={() => setRoleModal(false)} className="rounded-xl border border-white/10 px-4 py-2 text-xs">إلغاء</button><button disabled={!selectedRole || selectedRole === selectedUser.role || !!busy} onClick={() => void executePatch({ action: 'change_role', userId: selectedUser.id, role: selectedRole })} className="rounded-xl bg-[#FF2E4C] px-4 py-2 text-xs font-black disabled:opacity-40">تأكيد تغيير الدور</button></div></div></div>}

      {creditModal && selectedUser && <div className="fixed inset-0 z-[200] grid place-items-center bg-black/75 p-4"><form onSubmit={(e) => { e.preventDefault(); void executePatch({ action: 'grant_credits', userId: selectedUser.id, amount: Number(creditAmount), reason }); }} className="w-full max-w-lg rounded-3xl border border-white/10 bg-[#0d0f15] p-6"><h3 className="text-lg font-black">إضافة رصيد للمستخدم</h3><p className="mt-1 text-xs text-gray-500">{selectedUser.email}</p><input type="number" min="1" step="1" value={creditAmount} onChange={(e) => setCreditAmount(e.target.value)} placeholder="عدد النقاط" className="mt-5 w-full rounded-xl border border-white/10 bg-black/30 p-3" required /><textarea value={reason} onChange={(e) => setReason(e.target.value)} placeholder="سبب إضافة الرصيد (يسجل في Audit Log)" className="mt-3 min-h-24 w-full rounded-xl border border-white/10 bg-black/30 p-3" required /><div className="mt-4 flex justify-end gap-2"><button type="button" onClick={() => setCreditModal(false)} className="rounded-xl border border-white/10 px-4 py-2 text-xs">إلغاء</button><button disabled={!!busy} className="rounded-xl bg-emerald-600 px-4 py-2 text-xs font-black">إضافة الرصيد</button></div></form></div>}
