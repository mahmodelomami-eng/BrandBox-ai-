...
/**
 * Hydrates the existing App shell with the authenticated user's real profile
 * and dashboard metrics without changing the existing visual design.
 *
 * The App component is intentionally large and currently owns a legacy
 * in-memory reducer. This bridge keeps that UI intact while the displayed
 * identity/metrics are sourced from Supabase. It also publishes the same
 * payload as a DOM event so the reducer can be migrated incrementally later.
 */
export default function RealUserDataBridge({ children }) {
  ...
  useEffect(() => {
    ...
    const hydrateIdentity = (profile, authUser) => {
      ...
      if (fullName) {
        document.querySelectorAll('[data-brandbox-user-name]').forEach((node) => setText(node, fullName));
      }
      if (email) {
        document.querySelectorAll('[data-brandbox-user-email]').forEach((node) => setText(node, email));
      }
      ...
      try {
        ...
        const [profileResult, projectsResult, assetsResult, transactionsResult] = await Promise.all([
          ...
          supabase
            ...
            .select('id,amount,transaction_type,description,reference_type,reference_id,created_at')
            .eq('user_id', userId)
        ...
        // The current dashboard has no stable data attributes yet. Hydrate the
        // visible metric cards by their existing Arabic labels.
  ...
  return (
    <>
      {children}
      {error && (
        <div className="fixed bottom-4 right-4 z-[70] max-w-sm rounded-xl border border-amber-500/30 bg-[#121520] px-4 py-3 text-xs text-amber-200 shadow-2xl">
          تعذر مزامنة بيانات الحساب: {error}
        </div>
      )}
      {!ready && (
