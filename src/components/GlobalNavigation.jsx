...
import { Bell, CheckCheck, FolderOpen, LogIn, LogOut, Menu, UserPlus, X } from 'lucide-react';
  ...
  return (
    <header className="brandbox-global-nav fixed inset-x-0 top-0 z-[100] border-b border-white/5 bg-[#050506]/95 backdrop-blur-xl" dir="rtl">
      <div className="mx-auto flex min-h-20 max-w-[1600px] items-center gap-3 px-4 lg:px-6">
        ...
        <div className="mr-auto hidden shrink-0 items-center gap-2 md:flex">
          {!session?.user ? <>
            ...
            <button onClick={() => { const next = !notificationsOpen; setNotificationsOpen(next); if (next) loadNotifications(); }} className="relative rounded-xl border border-white/10 p-2.5 text-gray-300 hover:border-[#f31325]/40 hover:text-white" aria-label="الإشعارات">
              <Bell size={19}/>{unreadCount > 0 && <span className="absolute -left-1 -top-1 flex min-h-5 min-w-5 items-center justify-center rounded-full bg-[#f31325] px-1 text-[9px] font-black">{unreadCount > 99 ? '99+' : unreadCount}</span>}
            </button>
            <Link href="/?view=dashboard" className="flex items-center gap-2 rounded-xl border border-white/10 bg-white/[.03] px-2.5 py-2 hover:border-[#f31325]/35" title="لوحة التحكم">
      ...
      {session?.user && notificationsOpen && (
        <div className="absolute left-4 top-[72px] z-[120] w-[min(92vw,390px)] overflow-hidden rounded-2xl border border-white/10 bg-[#0d1016]/98 shadow-[0_25px_80px_rgba(0,0,0,.55)] backdrop-blur-xl">
          <div className="flex items-center justify-between border-b border-white/10 px-4 py-4"><div className="flex items-center gap-2 font-black"><Bell size={17} className="text-[#ff3344]"/> الإشعارات {unreadCount > 0 && <span className="rounded-full bg-[#f31325] px-2 py-0.5 text-[10px]">{unreadCount}</span>}</div><div className="flex gap-2"><button onClick={markAllRead} className="rounded-lg border border-white/10 p-2 text-gray-400 hover:text-white" title="تحديد الكل كمقروء"><CheckCheck size={15}/></button><button onClick={() => setNotificationsOpen(false)} className="rounded-lg border border-white/10 p-2 text-gray-400 hover:text-white"><X size={15}/></button></div></div>
          <div className="max-h-[420px] overflow-y-auto p-2">{notifications.length === 0 ? <div className="p-8 text-center text-sm text-gray-500">لا توجد إشعارات جديدة.</div> : notifications.map((item) => <div key={item.id} className={`mb-2 rounded-xl border p-4 ${item.is_read ? 'border-white/[.07] bg-white/[.02]' : 'border-[#f31325]/25 bg-[#f31325]/5'}`}><div className="flex items-start justify-between gap-3"><div className="text-sm font-black">{item.title}</div>{!item.is_read && <span className="mt-1.5 h-2 w-2 shrink-0 rounded-full bg-[#ff3344]"/>}</div>{item.body && <p className="mt-2 text-xs leading-6 text-gray-400">{item.body}</p>}<div className="mt-2 text-[10px] text-gray-600">{item.created_at ? new Date(item.created_at).toLocaleString('ar-LY') : ''}</div></div>)}</div>
        </div>
      )}

      {open && (
        <nav className="max-h-[calc(100vh-5rem)] overflow-y-auto border-t border-white/5 bg-[#090a0d] px-4 py-4 xl:hidden">
          <div className="mx-auto grid max-w-3xl gap-1 sm:grid-cols-2">{NAV_ITEMS.map(([label, href]) => <Link key={href} href={href} onClick={() => setOpen(false)} className="rounded-xl px-4 py-3 text-sm font-bold text-gray-300 transition hover:bg-white/5 hover:text-white">{label}</Link>)}</div>
          <div className="mx-auto mt-4 flex max-w-3xl flex-wrap gap-2 border-t border-white/10 pt-4 md:hidden">{!session?.user ? <><Link href="/auth?next=%2F" className="rounded-xl border border-white/10 px-4 py-3 text-sm font-black">تسجيل الدخول</Link><Link href="/auth?next=%2F" className="rounded-xl bg-[#f31325] px-4 py-3 text-sm font-black">اشتراك</Link></> : <><Link href="/?view=dashboard" onClick={() => setOpen(false)} className="rounded-xl border border-white/10 px-4 py-3 text-sm font-black">{displayName}</Link><Link href="/projects" onClick={() => setOpen(false)} className="rounded-xl border border-white/10 px-4 py-3 text-sm font-black">مشاريعي</Link><button onClick={signOut} className="rounded-xl border border-red-500/20 px-4 py-3 text-sm font-black text-red-300">تسجيل الخروج</button></>}</div>
        </nav>
      )}
    </header>
  );
