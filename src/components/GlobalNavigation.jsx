      ...
      <div className="mx-auto flex min-h-20 max-w-[1500px] items-center gap-5 px-4 lg:px-6">
        ...
        <nav className="hidden min-w-0 flex-1 items-center justify-center gap-1 xl:flex">
          {NAV_ITEMS.map(([label, href]) => {
            ...
            return (
              <Link key={href} href={href} className={`whitespace-nowrap rounded-lg px-2.5 py-2 text-[13px] font-bold transition ${active ? 'bg-[#f31325]/12 text-[#ff3344]' : 'text-gray-300 hover:bg-white/5 hover:text-white'}`}>
                {label}
              </Link>
            );
          })}
      ...
      {open && (
        <nav className="max-h-[calc(100vh-5rem)] overflow-y-auto border-t border-white/5 bg-[#090a0d] px-4 py-4 xl:hidden">
          <div className="mx-auto grid max-w-3xl gap-1 sm:grid-cols-2">
            {NAV_ITEMS.map(([label, href]) => (
              <Link key={href} href={href} onClick={() => setOpen(false)} className="rounded-xl px-4 py-3 text-sm font-bold text-gray-300 transition hover:bg-white/5 hover:text-white">
                {label}
              </Link>
            ))}
          </div>
