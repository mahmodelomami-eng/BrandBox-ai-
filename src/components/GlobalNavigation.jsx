...
function navClass(active) {
  return `whitespace-nowrap rounded-lg px-2.5 py-2 text-[13px] font-bold transition ${active
    ...
    <header className="brandbox-global-nav fixed inset-x-0 top-0 z-[100] border-b border-white/[.07] bg-[#050506] shadow-[0_10px_35px_rgba(0,0,0,.28)]" dir="rtl">
      <div className="mx-auto flex min-h-20 max-w-[1660px] items-center gap-3 px-4 lg:px-6">
        ...
        <nav className="hidden min-w-0 flex-1 items-center justify-center gap-1 xl:flex" aria-label="التنقل الرئيسي">
          <Link href="/" className={navClass(pathname === '/')}>الرئيسية</Link>
          <Link href={dashboardHref} className={navClass(pathname.startsWith('/dashboard'))}>لوحة تحكم المستخدم</Link>
              ...
              <div className="absolute right-0 top-full z-[130] w-72 pt-2" role="menu">
                <div className="rounded-2xl border border-white/10 bg-[#0d1016] p-2 shadow-[0_24px_70px_rgba(0,0,0,.72)]">
                  {AI_TOOLS.map(({ label, href, icon: Icon }) => (
                    <Link key={href} href={href} onClick={() => setAiOpen(false)} className="flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-bold text-gray-300 transition hover:bg-white/5 hover:text-white" role="menuitem">
          ...
          {NAV_ITEMS.map(([label, href]) => (
            <Link key={href} href={href} className={navClass(pathname === href || pathname.startsWith(`${href}/`))}>{label}</Link>
          ))}
        </nav>
            ...
            <>
              ...
              <div ref={accountRef} className="relative flex items-center gap-1">
                ...
                <button
                  ...
                  className="grid h-9 w-8 place-items-center rounded-lg text-gray-500 transition hover:bg-white/5 hover:text-white"
        ...
        <button type="button" onClick={() => setMobileOpen((value) => !value)} className="mr-auto rounded-xl p-2.5 text-white xl:hidden" aria-label="القائمة الرئيسية">{mobileOpen ? <X size={22} /> : <Menu size={22} />}</button>
      </div>

      {mobileOpen && (
        <nav className="max-h-[calc(100vh-5rem)] overflow-y-auto border-t border-white/[.07] bg-[#090a0d] px-4 py-4 xl:hidden" aria-label="التنقل على الهاتف">
          <div className="mx-auto max-w-3xl space-y-1">
            <Link href="/" onClick={() => setMobileOpen(false)} className="block rounded-xl px-4 py-3 text-sm font-bold text-gray-300 hover:bg-white/5">الرئيسية</Link>
            <Link href={dashboardHref} onClick={() => setMobileOpen(false)} className="flex items-center gap-2 rounded-xl px-4 py-3 text-sm font-bold text-gray-300 hover:bg-white/5"><LayoutDashboard size={17} className="text-[#ff3344]" /> لوحة تحكم المستخدم</Link>
            ...
            <div className="rounded-2xl border border-white/10 bg-[#0d1016] p-2">
              ...
              {AI_TOOLS.map(({ label, href, icon: Icon }) => (
                <Link key={href} href={href} onClick={() => setMobileOpen(false)} className="flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-bold text-gray-300 hover:bg-white/5 hover:text-white"><Icon size={17} className="text-[#ff3344]" /> {label}</Link>
              ))}
            </div>
