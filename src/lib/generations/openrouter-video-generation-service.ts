...
import type { StartVideoGenerationRequest, VideoGenerationResult } from './video-generation-service';
...
function normalizeVideoFailure(error: unknown): { code: string; message: string; transient: boolean } {
  ...
  if (raw.startsWith('VIDEO_OUTPUT_') || raw.startsWith('VIDEO_ASSET_')) {
...
async function refundVideoCredits(userId: string, generationId: string, amount: number, failureCode: string) {
...
export class OpenRouterVideoGenerationService {
  public static async start(
    ...
    if (
      !Number.isInteger(creditsPerSecond) || creditsPerSecond < 1
    ...
    const { data: existing } = await database.from('generations')
  ...
  public static async refresh(actor: AuthContext, id: string): Promise<VideoGenerationResult> {
    ...
    try {
      const bytes = await downloadOpenRouterVideoContent(task.taskId);
      ...
      const durationMs = generation.created_at ? Math.max(0, Date.now() - new Date(generation.created_at).getTime()) : null;
