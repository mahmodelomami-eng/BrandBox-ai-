...
export async function processDigitalCodeFulfillmentForOrder(orderId:string){
 ...
 for(const item of items||[]){
  ...
  const {data:job}=await db.from('store_fulfillment_jobs').select('id,status,attempt_count').eq('order_item_id',item.id).maybeSingle();
  if(!job) throw new Error('STORE_FULFILLMENT_JOB_NOT_FOUND');
  if(job.status==='SUCCEEDED'){processed++;continue;}
  await db.from('store_fulfillment_jobs').update({status:'PROCESSING',attempt_count:Number(job.attempt_count||0)+1,locked_at:new Date().toISOString(),updated_at:new Date().toISOString()}).eq('id',job.id).in('status',['PENDING','FAILED','REVIEW_REQUIRED']);
  ...
  if(deliveryError||!delivered||delivered.length<item.quantity){
    await db.from('store_fulfillment_jobs').update({status:'REVIEW_REQUIRED',last_error_code:'STORE_CODE_DELIVERY_FAILED',last_error_message:deliveryError?.message||'Digital code delivery returned an incomplete result.',locked_at:null,updated_at:new Date().toISOString()}).eq('id',job.id);
    await db.from('store_orders').update({status:'REVIEW_REQUIRED',updated_at:new Date().toISOString()}).eq('id',orderId).eq('payment_status','PAID');
    throw new Error('STORE_CODE_DELIVERY_FAILED');
  }
