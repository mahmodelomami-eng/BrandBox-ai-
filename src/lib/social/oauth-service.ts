...
async function exchangeMeta(
  ...
  try {
    ...
    // The short-lived token is still a valid connection and will be surfaced as expiring sooner.
  }
  ...
  try {
    ...
    scopes = permissionRows
      .map((item) => asObject(item))
      .filter((item) => item.status === 'granted')
      .map((item) => asString(item.permission))
      .filter(Boolean);
  } catch {
    // Requested scopes remain a conservative fallback until a future sync.
  }
...
async function discoverYouTubeAccounts(
  token: TokenBundle
): Promise<DiscoveredAccount[]> {
  const payload = await fetchJson(
...
async function exchangeAndDiscover(
  provider: SocialProviderId,
  code: string,
  requestedScopes: string[]
): Promise<{ token: TokenBundle; accounts: DiscoveredAccount[] }> {
  if (provider === 'meta') {
...
export async function failOAuthConnection(
  provider: SocialProviderId,
  state: string,
  reason: string
) {
...
export function defaultOAuthFailureRedirect(providerValue: string) {
  ...
  return appendReturnParams(mobileReturnUri(), {
    provider,
    status: 'error',
    reason: 'oauth_failed',
  });
...
export function providerConnectionCapability(
  provider: SocialProviderId,
