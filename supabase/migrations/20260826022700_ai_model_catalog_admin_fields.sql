...
ALTER TABLE public.ai_model_catalog
  ...
  ADD CONSTRAINT ai_model_catalog_tool_category_check
    CHECK (tool_category IN ('chat','image','video','audio','vision','agent','general'));

ALTER TABLE public.ai_model_catalog
  DROP CONSTRAINT IF EXISTS ai_model_catalog_minimum_plan_check,
  ADD CONSTRAINT ai_model_catalog_minimum_plan_check
    CHECK (minimum_plan_id IS NULL OR minimum_plan_id IN ('free','starter','pro','business'));
...
SET
  ...
  is_featured = TRUE,
  metadata = COALESCE(metadata, '{}'::jsonb) || jsonb_build_object('brandbox_alias','Brand Box Smart','show_real_model_name',true),
  updated_at = NOW()
WHERE model_id = 'google/gemini-3.7-flash';

CREATE INDEX IF NOT EXISTS idx_ai_model_catalog_visibility
  ON public.ai_model_catalog (generation_type, is_enabled, is_visible_to_users, sort_order);
