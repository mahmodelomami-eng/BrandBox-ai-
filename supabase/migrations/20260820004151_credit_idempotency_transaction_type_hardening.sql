...
ALTER TABLE public.credit_idempotency ADD COLUMN IF NOT EXISTS transaction_type TEXT;
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'credit_idempotency_transaction_type_check'
    AND conrelid = 'public.credit_idempotency'::regclass) THEN
    ALTER TABLE public.credit_idempotency ADD CONSTRAINT credit_idempotency_transaction_type_check
      CHECK (transaction_type IS NULL OR transaction_type IN ('deduction','refund','grant','subscription','purchase','admin_adjustment'));
  END IF;
END $$;
...
BEGIN
  ...
  IF v_existing.idempotency_key IS NOT NULL THEN
    IF v_existing.user_id IS DISTINCT FROM p_user_id OR v_existing.action IS DISTINCT FROM 'grant'
       OR v_existing.amount IS DISTINCT FROM p_amount
       OR (v_existing.transaction_type IS NOT NULL AND v_existing.transaction_type IS DISTINCT FROM p_tx_type)
  ...
  VALUES (p_idempotency_key, p_user_id, 'grant', p_amount, v_tx_id, p_tx_type) ON CONFLICT (idempotency_key) DO NOTHING;
  ...
  INSERT INTO public.credit_transactions (id, user_id, amount, transaction_type, description,
    reference_type, reference_id, actor_id, idempotency_key)
  VALUES (v_tx_id, p_user_id, p_amount, p_tx_type, p_description,
    p_reference_type, p_reference_id, COALESCE(p_actor_id, p_user_id), p_idempotency_key);
  RETURN QUERY SELECT TRUE, (v_current_balance + p_amount), 'SUCCESS'::TEXT, v_tx_id;
END;
$$;

REVOKE ALL ON FUNCTION public.grant_credits_idempotent(UUID, INTEGER, TEXT, TEXT, TEXT, TEXT, UUID, TEXT)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.grant_credits_idempotent(UUID, INTEGER, TEXT, TEXT, TEXT, TEXT, UUID, TEXT)
  TO service_role;
