...
-- Browser presentation remains driven by server catalog fields. The canonical
-- flag stays in is_featured; brandbox_featured mirrors it into the already-safe
-- metadata payload consumed by the current media APIs.
...
SET is_featured = CASE model_id
    ...
    metadata = COALESCE(metadata, '{}'::jsonb) || jsonb_build_object(
      'brandbox_featured', CASE model_id
        WHEN 'google/gemini-3.1-flash-image' THEN TRUE
        WHEN 'google/gemini-3.1-flash-lite-image' THEN TRUE
        WHEN 'openai/gpt-image-2' THEN TRUE
        WHEN 'bytedance-seed/seedream-4.5' THEN TRUE
